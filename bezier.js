...
class PolyBezier {
	...
	length_f(f) {
		...
		if (id < 0)
			id = 0;
		if (id > this.curves.length)
			id = this.curves.length;
		f -= id;
		for (var i = 0; i < id; i++)
			l += this.curves[i].length();
		if (f===0 || id === this.curves.length)
			return l
		l += this.curves[i].length_f(f);
		return l
	}
	...
	get(f) {
		...
		if (id < 0)
			id = 0;
		if (id >= this.curves.length)
			id = this.curves.length - 1;
		f -= id;
		return this.curves[id].get(f)
	}
}
...
class Bezier {
	...
	length_f(f) {
		...
		if (f === 1)
			return this.length();
		return this.split(f).left.length();
	}
...
function Spline(points){
	var n = (points.length - 1)/3
	var curves = []
	for (var i = 0; i<n; i++){
		var a = points.slice(3*i, 3*i+4);
		console.log(a);
		curves.push(new Bezier(...a.flat()));
	}
	return new PolyBezier(curves)
}
